(function ($) {
    youQiu.Upload = function (options) {
        ...
        var imagePanel = $('<div class="clearfix"></div>');
        ...
        $(obj).append(imagePanel).append('<p class="unchecked" style="display:none"></p><span id="' + buttonSpan + '"></span>');
        ...
        var swfUpload = new SWFUpload({
            ...
            swfupload_loaded_handler: function () {
                ...
                stats.successful_uploads = $(obj).find('.upfile').length;
        ...
        function fileDialogStart() {
            $(obj).find('.unchecked').hide();
        }

        function fileQueueError(file, errorCode, message) {
            $(obj).find('.unchecked').html('最多上传' + options.limit + '张图片,每张图片不超过5M');
            $(obj).find('.unchecked').show();
        }

        function fileQueue(file) {
            var image = $(['<div class="fL mgL10 upfile" id="thumb_' + file.id + '">',
                '<div class="upload_zhizhao">',
                  '<img src="', appUrl, 'Content/images/base/loading.gif" style="margin: 32px" width="32" height="32" alt=""/>',
                '</div>',
                '<p>处理中…</p>',
              '</div>'].join(''));
            $(obj).find('.clearfix').append(image);
            image.fadeIn(500);
        }
        ...
        function lightBox() {
            $(obj).find('.upload_zhizhao>a').lightBox({
        ...
        function generateImageHtml(image) {
            return $(['<div class="fL mgL10 upfile">',
                '<div class="upload_zhizhao">',
                '<a href="', image.OriginalUrl, '">',
                '<img src="', image.ThumbnailUrl, '" width="96" height="96" /></a></div>',
                '<div><p>',
                '<img src="', appUrl, 'Content/images/base/icon_gou.gif" width="16" height="16" alt=""/>',
                '上传成功!',
                '<a href="#" class="deleteImage" fileId="' + (image.fileId || '') + '" fileName="' + image.Id + '">',
                '<img src="', appUrl, 'Content/images/base/icon_ljt.gif" alt="删除" width="16" height="16" /></a></p>'].join(''));
        }
    };
